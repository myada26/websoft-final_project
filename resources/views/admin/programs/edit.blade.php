    ...
    <div style="max-width:560px">
        <div style="background:white;border-radius:12px;border:1px solid #dde8e1;box-shadow:0 1px 2px rgba(0,0,0,.06)">
            <div style="padding:16px 20px;border-bottom:1px solid #eaf0ec">
                <div style="font-size:15px;font-weight:700">Edit Program</div>
                <div style="font-size:12px;color:#8aa89a;margin-top:2px">Update program details</div>
            </div>
            <div style="padding:24px">
                <form method="POST" action="{{ route('admin.programs.update', $program) }}">
                    ...
                    <div style="margin-bottom:16px">
                        ...
                        <select name="department_id" style="width:100%;padding:9px 12px;border:1.5px solid {{ $errors->has('department_id') ? '#fca5a5' : '#dde8e1' }};border-radius:8px;font-size:13.5px;outline:none;box-sizing:border-box;background:white" required>
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->id }}" {{ old('department_id', $program->department_id) == $dept->id ? 'selected' : '' }}>{{ $dept->code }} — {{ $dept->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div style="margin-bottom:16px">
                        ...
                        <input type="text" name="code" value="{{ old('code', $program->code) }}" maxlength="20"
