...
<div class="max-w-6xl mx-auto pb-10" x-data="{ open: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
            <h2 class="text-[22px] font-bold text-[#0f1f17]">Manage Programs</h2>
            <p class="text-[13.5px] text-[#4a6356] mt-1 font-medium">Programs belong to departments (Level 3 — Membership scope)</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[#dde8e1] shadow-sm overflow-hidden">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-6 py-5 border-b border-[#eaf0ec]">
            <div>
                <h3 class="text-[15px] font-bold text-[#0f1f17]">All Programs</h3>
                <p class="text-[12.5px] text-[#8aa89a] font-medium mt-0.5">{{ $programs->total() }} total records</p>
            </div>
            <form method="GET" action="{{ route('admin.programs.index') }}" class="flex flex-wrap items-center gap-2.5">
                ...
                <select name="department_id" onchange="this.form.submit()" class="border-2 border-[#dde8e1] rounded-xl py-2 px-3 text-[13px] font-medium text-[#4a6356] outline-none focus:border-[#1a7a41] bg-white cursor-pointer transition-colors">
            ...
            <table class="w-full text-left border-collapse min-w-[700px]">
                <thead>
                    <tr class="bg-[#f8fbf9] border-b border-[#dde8e1]">
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-[#8aa89a] uppercase tracking-widest">Code</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-[#8aa89a] uppercase tracking-widest">Program Name</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-[#8aa89a] uppercase tracking-widest">Department</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-[#8aa89a] uppercase tracking-widest">Students</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0 group">
                        <td class="px-6 py-4"><span class="font-mono text-[13px] font-bold text-[#1a7a41] bg-[#e6f4ec] px-2 py-1 rounded-md">{{ $prog->code }}</span></td>
                        <td class="px-6 py-4 text-[14px] font-bold text-[#0f1f17]">{{ $prog->name }}</td>
                        <td class="px-6 py-4 text-[13.5px] font-semibold text-[#4a6356]">{{ $prog->department->code ?? '—' }}</td>
                        <td class="px-6 py-4 text-[13.5px] font-semibold text-[#4a6356]">{{ $prog->enrollments_count ?? '—' }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.programs.edit', $prog) }}" class="p-1.5 rounded-lg text-[#8aa89a] hover:bg-[#dde8e1] hover:text-[#1a7a41] transition-colors">
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-14 text-center text-[14px] font-semibold text-[#4a6356]">No programs found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="px-6 py-4 border-t border-[#eaf0ec] flex justify-between items-center bg-[#f8fbf9]">
            <span class="text-[12.5px] font-medium text-[#8aa89a]">Showing {{ $programs->firstItem() ?? 0 }}–{{ $programs->lastItem() ?? 0 }} of {{ $programs->total() }}</span>
    ...
    <div x-show="open === 'manual'" x-cloak class="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative bg-white rounded-2xl w-full max-w-lg shadow-2xl z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#eaf0ec] shrink-0">
                <div><h2 class="text-[18px] font-bold text-[#0f1f17]">Add New Program</h2><p class="text-[13px] text-[#4a6356] mt-0.5 font-medium">Create a program within a department.</p></div>
            ...
            <form method="POST" action="{{ route('admin.programs.store') }}" class="flex flex-col min-h-0">
                ...
                <div class="p-6 overflow-y-auto space-y-5">
                    <div>
                        <label class="block text-[13px] font-semibold text-[#4a6356] mb-2">Program Code <span class="text-red-500">*</span></label>
                        <input type="text" name="code" value="{{ old('code') }}" placeholder="e.g. BSCS" class="w-full px-4 py-2.5 border-2 border-[#dde8e1] rounded-xl bg-[#f8fbf9] text-[14px] font-bold font-mono text-[#0f1f17] outline-none focus:border-[#1a7a41] transition-colors uppercase">
                    </div>
                    <div>
                        <label class="block text-[13px] font-semibold text-[#4a6356] mb-2">Program Name <span class="text-red-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. BS Computer Science" class="w-full px-4 py-2.5 border-2 border-[#dde8e1] rounded-xl bg-white text-[14px] font-medium text-[#0f1f17] outline-none focus:border-[#1a7a41] transition-colors">
                    </div>
                    <div>
                        <label class="block text-[13px] font-semibold text-[#4a6356] mb-2">Parent Department <span class="text-red-500">*</span></label>
                ...
                <div class="px-6 py-4 border-t border-[#eaf0ec] bg-[#f8fbf9] flex justify-end gap-3 shrink-0 rounded-b-2xl">
                    ...
                    <button type="submit" class="px-4 py-2 rounded-xl text-[13.5px] font-bold bg-[#1a7a41] hover:bg-[#27a05a] text-white border-2 border-transparent transition-all shadow-sm">Save Program</button>
                </div>
            </form>
        </div>
    </div>
