@extends('layouts.app')
@section('title', 'Edit Fee Profile')
@section('page-title', 'Edit Fee Profile')
    ...
    <div style="max-width:520px">
        <div style="background:white;border-radius:12px;border:1px solid #dde8e1;box-shadow:0 1px 2px rgba(0,0,0,.06)">
            <div style="padding:16px 20px;border-bottom:1px solid #eaf0ec">
                <div style="font-size:15px;font-weight:700">Edit Fee Profile</div>
                <div style="font-size:12px;color:#8aa89a;margin-top:2px">{{ $feeProfile->name }}</div>
            </div>
            <div style="padding:24px">
                <form method="POST" action="{{ route('org.fee-profiles.update', $feeProfile) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div style="margin-bottom:16px">
                        ...
                        <input type="text" name="name" value="{{ old('name', $feeProfile->name) }}"
                    ...
                    <div style="margin-bottom:16px">
                        ...
                        <select name="category" style="width:100%;padding:9px 12px;border:1.5px solid #dde8e1;border-radius:8px;font-size:13.5px;outline:none;box-sizing:border-box;background:white" required>
                            ...
                            <option value="{{ $cat }}" {{ old('category', $feeProfile->category) === $cat ? 'selected' : '' }}>{{ ucfirst(strtolower($cat)) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div style="margin-bottom:16px">
                        ...
                        <div style="position:relative">
                            ...
                            <input type="number" name="amount" value="{{ old('amount', $feeProfile->amount) }}" step="0.01" min="0"
                    ...
                    <div style="margin-bottom:16px">
                        <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $feeProfile->is_active) ? 'checked' : '' }} style="width:15px;height:15px;accent-color:#27a05a">
