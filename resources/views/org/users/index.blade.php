...
<div class="max-w-6xl mx-auto pb-10" x-data="{ open: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-[13.5px] text-[#4a6356] mt-1 font-medium">Manage user accounts for {{ auth()->user()->organization?->name ?? 'your organization' }}</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[#dde8e1] shadow-sm overflow-hidden">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-6 py-5 border-b border-[#eaf0ec]">
            <div>
                <h3 class="text-[15px] font-bold text-[#0f1f17]">Organization Users</h3>
                <p class="text-[12.5px] text-[#8aa89a] font-medium mt-0.5">{{ $users->total() }} total members</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse min-w-[700px]">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0 group">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('org.users.edit', $user) }}" class="p-1.5 rounded-lg text-[#8aa89a] hover:bg-[#dde8e1] hover:text-[#1a7a41] transition-colors">
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-14 text-center text-[14px] font-semibold text-[#4a6356]">No users found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative bg-white rounded-2xl w-full max-w-2xl shadow-2xl z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#eaf0ec] shrink-0">
                <div><h2 class="text-[18px] font-bold text-[#0f1f17]">Invite New User</h2><p class="text-[13px] text-[#4a6356] mt-0.5 font-medium">Create a new account for your organization.</p></div>
            ...
            <form method="POST" action="{{ route('org.users.store') }}" class="flex flex-col min-h-0">
                ...
                <div class="p-6 overflow-y-auto space-y-6">
                    <div>
                        ...
                        <div>
                            <label class="block text-[13px] font-semibold text-[#4a6356] mb-2">Email Address</label>
                    ...
                    <div>
                        <h3 class="text-[14px] font-bold text-[#1a7a41] mb-4 pb-2 border-b border-[#dde8e1]">Access Level</h3>
                        <div class="mb-4">
                            <label class="block text-[13px] font-semibold text-[#4a6356] mb-2">Username <span class="text-red-500">*</span></label>
                            <input type="text" name="username" value="{{ old('username') }}" placeholder="e.g. juan.treasurer" class="w-full px-4 py-2.5 border-2 border-[#dde8e1] rounded-xl bg-[#f8fbf9] text-[14px] font-bold font-mono text-[#0f1f17] outline-none focus:border-[#1a7a41] transition-colors">
                        </div>
                        <div class="mb-4">
                ...
                <div class="px-6 py-4 border-t border-[#eaf0ec] bg-[#f8fbf9] flex justify-end gap-3 shrink-0 rounded-b-2xl">
                    ...
                    <button type="submit" class="px-4 py-2 rounded-xl text-[13.5px] font-bold bg-[#1a7a41] hover:bg-[#27a05a] text-white border-2 border-transparent transition-all shadow-sm">Create Account</button>
                </div>
            </form>
        </div>
    </div>
