...
<div class="page-shell" x-data="{ open: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        ...
        @if(auth()->user()->isAdmin() || auth()->user()->canManageOrgUsers())
        <a href="{{ route('org.users.create') }}" class="px-4 py-2 rounded-lg text-[13.5px] font-bold flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white border-2 border-transparent transition-all shadow-sm">
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0 group">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                @if(auth()->id() !== $user->id)
                                <form method="POST" action="{{ route('org.users.destroy', $user) }}" data-confirm-title="Remove user from organization?" data-confirm-message="{{ $user->username }} will be removed. This cannot be undone." data-confirm-label="Remove" data-confirm-tone="danger">
                                    @csrf @method('DELETE')
